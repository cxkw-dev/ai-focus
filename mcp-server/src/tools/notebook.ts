...
export function registerNotebookTools(server: McpServer) {
  ...
  server.tool(
    "list_notes",
    "List notebook notes. Optionally search by title. Returns titles and IDs (use get_note for full content).",
    {
  ...
  server.tool(
    "update_note",
    "Update a notebook note's title, content, or pinned status.",
    {
      id: z.string().describe("The note ID"),
  ...
  server.tool(
    "delete_note",
    "Delete a notebook note permanently.",
    {
  ...
  server.tool(
    "update_scratchpad",
    "Update the scratch pad content. Content supports markdown.",
    {
