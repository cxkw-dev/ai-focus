...
export function registerTodoTools(server: McpServer) {
  server.tool(
    'list_todos',
    'List todos from AI Focus. Returns a compact summary by default; set verbose=true for full JSON with IDs and descriptions.',
    {
      status: z
        .enum([
          'TODO',
      ...
      completed: z
        ...
        .optional()
        .describe(
          "Filter by completion: 'true' for completed todos, 'false' for not completed",
        ),
      excludeStatus: z
        .enum([
          'TODO',
      ...
      limit: z
        ...
        .optional()
        .describe(
          'Max number of todos to return (1-100). When set, the total count is also reported.',
        ),
      offset: z
        .number()
        .int()
      ...
      verbose: z
        ...
        .optional()
        .describe(
          'Return full JSON instead of compact summary (default false)',
        ),
    },
    async (params) => {
      ...
      const isPaginated =
        ...
        'todos' in data
      const todos = (
        isPaginated ? (data as { todos: TodoResponse[] }).todos : data
      ) as TodoResponse[]
      const summary = formatTodoSummary(todos)
      const text = isPaginated
        ? `${summary}\n\n(showing ${todos.length} of ${(data as { total: number }).total} total)`
        : summary
      return {
  ...
  server.tool(
    'get_todo_board',
    'Get the todo board grouped into three buckets: active (not completed/cancelled), completed (completed or cancelled), and deleted (archived but not completed/cancelled). Returns a readable summary of each bucket.',
    {},
    async () => {
  ...
  server.tool(
    'get_todo',
    'Get a single todo by task number (e.g. 7) or ID. Returns full details including description, subtasks, labels, and linked PRs.',
    {
      taskNumber: z
        .number()
        .int()
...
- For timeline/deadline info, use the dueDate field — never put dates or deadlines in the description.`,
    {
      ...
      description: z
        ...
        .optional()
        .describe(
          'One-sentence summary of the task (~150 chars). Do NOT put detailed context, requirements, phases, or timelines here — use a linked notebook note for details and dueDate for deadlines.',
        ),
      priority: z
        .enum(['LOW', 'MEDIUM', 'HIGH', 'URGENT'])
        .optional()
      ...
      notebookNoteId: z
        ...
        .optional()
        .describe(
          'Notebook note ID to link. Create a note first via create_note, then pass its ID here for detailed context.',
        ),
    },
    async (params) => {
...
- Use get_todo first if you need to see the current description.`,
    {
      ...
      description: z
        ...
        .optional()
        .describe(
          'One-sentence summary (behavior depends on descriptionMode). Keep brief — detailed context belongs in a linked notebook note.',
        ),
      descriptionMode: z
        .enum(['append', 'replace'])
        .optional()
        .describe(
          "'append' (default) adds to existing content; 'replace' overwrites entirely",
        ),
      status: z
        .enum([
          'TODO',
      ...
      subtasks: z
        ...
        .optional()
        .describe(
          'Replace all subtasks (declarative sync). Include id for existing subtasks, omit for new ones.',
        ),
      myPrUrls: z
        .array(z.string())
        .optional()
    ...
    async ({ taskNumber, id, descriptionMode, ...updates }) => {
      ...
      if (updates.description) {
        ...
        if ((descriptionMode ?? 'append') === 'append') {
          const existing = (await apiFetch(`/api/todos/${key}`)) as {
  ...
  server.tool(
    'delete_todo',
    'Permanently delete a todo. This cannot be undone — use archive_todo for soft delete instead.',
    {
      taskNumber: z
        .number()
        .int()
  ...
  server.tool(
    'toggle_subtask',
    "Toggle a subtask's completed status within a todo.",
    {
      taskNumber: z
        .number()
        .int()
      ...
      todoId: z
        ...
        .optional()
        .describe(
          'The parent todo cuid (use taskNumber instead when possible)',
        ),
  ...
  server.tool(
    'search_todos',
    'Search active todos by title. Uses server-side filtering for efficiency. Returns a compact summary of matching todos.',
    {
      query: z
        .string()
        .min(1)
        .describe(
          'Search text to match against todo titles (case-insensitive)',
        ),
    },
    async ({ query }) => {
      const data = await apiFetch(
        `/api/todos?search=${encodeURIComponent(query)}`,
      )
      if (isApiError(data)) return textResult(data)
      return {
        content: [
          {
            type: 'text' as const,
            text: formatTodoSummary(data as TodoResponse[]),
          },
        ],
      }
    },
  ...
  server.tool(
    'archive_todo',
    'Archive a todo (soft delete). The todo can be restored later with restore_todo.',
    {
      taskNumber: z
        .number()
        .int()
  ...
  server.tool(
    'restore_todo',
    'Restore an archived todo back to the active list.',
    {
      taskNumber: z
        .number()
        .int()
  ...
  server.tool(
    'complete_todo',
    'Mark a todo as completed. This also auto-archives it. Shortcut for update_todo with status=COMPLETED.',
    {
      taskNumber: z
        .number()
        .int()
  ...
  server.tool(
    'start_todo',
    'Mark a todo as in progress. Shortcut for update_todo with status=IN_PROGRESS.',
    {
      taskNumber: z
        .number()
        .int()
  ...
  server.tool(
    'reorder_todos',
    'Reorder active todos by providing an array of todo IDs in the desired order. Use list_todos with verbose=true first to get the IDs.',
    {
      orderedIds: z
