...
- For timeline/deadline info, use the dueDate field — never put dates or deadlines in the description.`,
    {
      ...
      subtasks: z.array(z.string()).optional().describe("Array of subtask titles to create"),
      myPrUrls: z.array(z.string()).optional().describe("GitHub PR URLs for this task's own PRs"),
      githubPrUrls: z.array(z.string()).optional().describe("Dependency GitHub PR URLs to wait on"),
      azureWorkItemUrl: z.string().nullable().optional().describe("Azure DevOps work item URL"),
...
- Use get_todo first if you need to see the current description.`,
    {
      ...
      dueDate: z.string().nullable().optional().describe("New due date (ISO string) or null to clear"),
      ...
      myPrUrls: z.array(z.string()).optional().describe("GitHub PR URLs for this task's own PRs"),
      githubPrUrls: z.array(z.string()).optional().describe("Dependency GitHub PR URLs to wait on"),
      azureWorkItemUrl: z.string().nullable().optional().describe("Azure DevOps work item URL (null to clear)"),
      azureDepUrls: z.array(z.string()).optional().describe("Dependent Azure DevOps work item URLs"),
      myIssueUrls: z.array(z.string()).optional().describe("GitHub Issue URLs for this task's own issues"),
      githubIssueUrls: z.array(z.string()).optional().describe("Dependency GitHub Issue URLs to wait on"),
      notebookNoteId: z.string().nullable().optional().describe("Notebook note ID to link (null to unlink)"),
    },
  ...
  server.tool(
    ...
    async (params) => {
      const data = await apiFetch(`/api/todos/${resolveKey(params)}`, { method: "DELETE" });
      return textResult(data);
    }
  );
  ...
  server.tool(
    ...
    {
      query: z.string().min(1).describe("Search text to match against todo titles (case-insensitive)"),
    },
    async ({ query }) => {
