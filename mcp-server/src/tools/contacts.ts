...
export function registerContactTools(server: McpServer) {
  ...
  server.tool(
    'update_todo_contact',
    'Update a contact on a todo — change their role and/or display order. Use list_todo_contacts first to find the contact ID.',
    {
      taskNumber: z
        .number()
        .int()
    ...
    async ({ taskNumber, id, contactId, role, order }) => {
      ...
      const data = await apiFetch(
        `/api/todos/${result.resolvedId}/contacts/${contactId}`,
        {
          method: 'PATCH',
  ...
  server.tool(
    ...
    async (params) => {
      ...
      if ('error' in result) return result.error
      const data = await apiFetch(
        `/api/todos/${result.resolvedId}/contacts/${params.contactId}`,
        {
          method: 'DELETE',
        },
      )
