...
function formatUpdates(updates: StatusUpdateResponse[]): string {
  return updates
    .map((u) => {
...
- "Deployed to staging, running smoke tests"`,
  ...
  server.tool(
    ...
    async ({ taskNumber, id }) => {
      ...
      return { content: [{ type: "text" as const, text: formatUpdates(updates) }] };
    }
  );
  ...
  server.resource(
    ...
    async (uri, variables) => {
      ...
      return {
        contents: [{ uri: uri.href, mimeType: "text/plain", text: formatUpdates(updates) }],
      };
    }
  );
}
