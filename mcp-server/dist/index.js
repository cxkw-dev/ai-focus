...
function parseAzureWorkItemId(url) {
    if (!url)
        return null;
...
// ─── Todos ───
server.tool("list_todos", "List todos/tasks from AI Focus. Returns a compact summary by default. Use verbose=true for full JSON.", {
    status: z
    ...
    if (params.verbose)
        return textResult(data);
    return {
        content: [{ type: "text", text: formatTodoSummary(data) }],
    };
});
server.tool("get_todo", "Get a single todo/task by task number (e.g. 7) or ID.", {
...
server.tool("create_todo", "Create a new todo/task in AI Focus.", {
...
server.tool("update_todo", `Update an existing todo/task. Use this to update progress, change status, edit description, etc.
...
server.tool("delete_todo", "Permanently delete a todo/task.", {
    taskNumber: z.number().int().positive().optional().describe("The task number (e.g. 7)"),
    id: z.string().optional().describe("The todo cuid (use taskNumber instead when possible)"),
}, async ({ taskNumber, id }) => {
    const key = taskNumber?.toString() ?? id;
...
// ─── People ───
server.tool("list_people", "List all people in the directory (name + email). Used for @mentions in task descriptions.", {}, async () => {
    const data = await apiFetch("/api/people");
    return textResult(data);
});
server.tool("create_person", "Add a person to the directory.", {
...
server.tool("delete_person", "Remove a person from the directory.", {
...
// ─── Notebook ───
server.tool("list_notes", "List notebook notes. Optionally search by title.", {
...
server.tool("delete_note", "Delete a notebook note.", {
...
// ─── Scratch Pad ───
server.tool("get_scratchpad", "Get the scratch pad content. A single persistent note for quick thoughts.", {}, async () => {
...
// ─── Azure DevOps ───
server.tool("get_azure_work_item", "Get Azure DevOps work item details for planning: title, state, assignee, area/iteration, description, acceptance criteria, tags, and links.", {
...
server.tool("get_azure_work_item_comments", "Get ordered Azure DevOps work item comments with author and timestamp.", {
    workItemId: z.number().int().positive().describe("Azure DevOps work item ID"),
}, async ({ workItemId }) => {
    const data = await apiFetch(`/api/azure/workitems/${workItemId}/comments`);
    return textResult(data);
});
server.tool("get_azure_work_item_relations", "Get Azure DevOps work item relations (parent/child/depends-on/related) with IDs, titles, and states.", {
    workItemId: z.number().int().positive().describe("Azure DevOps work item ID"),
}, async ({ workItemId }) => {
    const data = await apiFetch(`/api/azure/workitems/${workItemId}/relations`);
    return textResult(data);
});
server.tool("get_azure_work_item_updates", "Get recent Azure DevOps work item field/status updates (who changed what and when).", {
    workItemId: z.number().int().positive().describe("Azure DevOps work item ID"),
}, async ({ workItemId }) => {
    const data = await apiFetch(`/api/azure/workitems/${workItemId}/updates`);
    return textResult(data);
});
server.tool("get_azure_pr_links", "Get Azure DevOps work item linked PRs/commits and PR merge status.", {
    workItemId: z.number().int().positive().describe("Azure DevOps work item ID"),
}, async ({ workItemId }) => {
...
server.tool("get_todo_execution_context", "Get a todo by task number or id and hydrate linked Azure planning context in one call. Optimized for kickstarting implementation work.", {
    taskNumber: z.number().int().positive().optional().describe("The task number (e.g. 7)"),
    id: z.string().optional().describe("The todo cuid (use taskNumber instead when possible)"),
    includeComments: z
        .boolean()
    ...
    maxUpdates: z
        ...
        .optional()
        .describe("Max Azure updates to return when includeUpdates=true (default 20)"),
}, async ({ taskNumber, id, includeComments = true, includeUpdates = false, maxComments = 20, maxUpdates = 20, }) => {
    const key = taskNumber?.toString() ?? id;
    if (!key)
        throw new Error("Provide either taskNumber or id");
    const todoData = await apiFetch(`/api/todos/${key}`);
    if (isApiError(todoData))
        return textResult(todoData);
    const todo = todoData;
...
server.tool("search_todos", "Search todos by title or description text. Returns matching active todos.", {
    ...
    return {
        content: [{ type: "text", text: formatTodoSummary(filtered) }],
    };
});
server.tool("archive_todo", "Archive a todo (soft delete). The todo can be restored later.", {
    taskNumber: z.number().int().positive().optional().describe("The task number (e.g. 7)"),
...
server.tool("restore_todo", "Restore an archived todo back to active.", {
    taskNumber: z.number().int().positive().optional().describe("The task number (e.g. 7)"),
    id: z.string().optional().describe("The todo cuid"),
}, async ({ taskNumber, id }) => {
    const key = taskNumber?.toString() ?? id;
...
server.tool("start_todo", "Mark a todo as in progress.", {
    taskNumber: z.number().int().positive().optional().describe("The task number (e.g. 7)"),
    id: z.string().optional().describe("The todo cuid"),
}, async ({ taskNumber, id }) => {
    const key = taskNumber?.toString() ?? id;
