...
- "Deployed to staging, running smoke tests"`, {
    ...
    server.tool('delete_status_update', "Delete a single status update from a todo's timeline. Use list_status_updates first to find the update ID.", {
        taskNumber: z
            .number()
            .int()
            .positive()
        ...
        if ('error' in resolved)
            return resolved.error;
        const data = await apiFetch(`/api/todos/${resolved.resolvedId}/updates/${updateId}`, {
            method: 'DELETE',
        });
        return textResult(data);
    });
