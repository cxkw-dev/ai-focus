...
export function registerPeopleTools(server) {
    server.tool("list_people", "List all people in the directory (name + email). People are used for @mentions in task descriptions and as todo contacts.", {}, async () => {
        const data = await apiFetch("/api/people");
        return textResult(data);
    });
    server.tool("create_person", "Add a person to the directory. Their email must be unique.", {
    ...
    server.tool("update_person", "Update a person's name or email.", {
        id: z.string().describe("The person's ID"),
        name: z.string().min(1).max(100).optional().describe("New name"),
        email: z.string().email().max(200).optional().describe("New email address"),
    }, async ({ id, ...updates }) => {
