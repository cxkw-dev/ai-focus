...
export function registerSessionTools(server) {
    server.tool('add_session', 'Attach an AI coding session to a todo so the user can resume it later. Call this when you start working on a task.', {
        taskNumber: z
            .number()
            .int()
            .positive()
    ...
    server.tool('remove_session', 'Remove an AI session from a todo.', {
