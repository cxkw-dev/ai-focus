...
export function registerAccomplishmentTools(server) {
    server.tool("list_accomplishments", "List accomplishments for a given year. Used for performance review tracking. Shows date, category, title, and description.", {
    ...
    server.tool("create_accomplishment", "Create a new accomplishment for performance review tracking.", {
        title: z.string().min(1).max(200).describe("Accomplishment title"),
        description: z.string().max(1000).optional().describe("Details (supports markdown)"),
        category: z
            .enum(["DELIVERY", "HIRING", "MENTORING", "COLLABORATION", "GROWTH", "OTHER"])
            .describe("Category: DELIVERY (features/PRs), HIRING (interviews), MENTORING (coaching), COLLABORATION (cross-team), GROWTH (learning), OTHER"),
    ...
    server.tool("update_accomplishment", "Update an existing accomplishment's title, description, category, or date.", {
    ...
    server.tool("delete_accomplishment", "Delete an accomplishment permanently.", {
