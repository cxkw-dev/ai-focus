...
const WORKITEM_URL_REGEX = /^https?:\/\/dev\.azure\.com\/([^/]+)\/([^/]+)\/_workitems\/edit\/(\d+)/
...
export async function GET(request: NextRequest) {
  ...
  if (!match) {
    return NextResponse.json({ error: 'Invalid Azure DevOps work item URL' }, { status: 400 })
  }
  ...
  if (!pat) {
    return NextResponse.json({ error: 'AZURE_DEVOPS PAT not configured' }, { status: 500 })
  }
  ...
  try {
    ...
    for (const rel of relations) {
      ...
      if (relUrl.includes('/git/repositories/') && relUrl.includes('/pullRequests/')) {
        linkedPrs.push({ name: name || 'Pull Request', url: relUrl })
      } else if (relUrl.includes('/git/repositories/') && relUrl.includes('/refs/')) {
        linkedBranches.push({ name: name || 'Branch', url: relUrl })
      } else if (relUrl.includes('/git/repositories/') && relUrl.includes('/commits/')) {
        ...
        if (relUrl.includes('PullRequestId') || comment.toLowerCase().includes('pull request')) {
          linkedPrs.push({ name: comment || name || 'Pull Request', url: relUrl })
        } else if (relUrl.includes('Ref') || comment.toLowerCase().includes('branch')) {
          linkedBranches.push({ name: comment || name || 'Branch', url: relUrl })
        } else if (relUrl.includes('Commit') || comment.toLowerCase().includes('commit')) {
    ...
    return NextResponse.json({ error: 'Failed to fetch work item status' }, { status: 502 })
  }
}
