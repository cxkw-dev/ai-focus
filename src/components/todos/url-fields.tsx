...
export function AzureIcon({ className, style }: { className?: string; style?: React.CSSProperties }) {
  return (
    <svg viewBox="0 0 16 16" fill="currentColor" className={className} style={style}>
...
export function SingleUrlField({
  ...
  return (
    <div
      className="flex items-center gap-2 rounded-md px-2.5 py-1.5 border transition-colors focus-within:border-[var(--primary)]"
      style={{
    ...
    <div className="space-y-1.5">
      {urls.map((url, index) => (
        <div key={url} className="flex items-center gap-2 group/dep">
          ...
          <button
            ...
            className="flex-shrink-0 opacity-0 group-hover/dep:opacity-100 transition-opacity"
            style={{ color: 'var(--text-muted)' }}
          >
            <X className="h-3.5 w-3.5" />
          </button>
      ...
      >
        ...
        <input
          ...
          disabled={disabled}
          className={`flex-1 bg-transparent ${textSize} focus:outline-none min-w-0`}
          style={{ color: 'var(--text-primary)' }}
        />
        <button
          type="button"
          ...
          className="flex-shrink-0 text-xs font-medium px-1 rounded transition-colors disabled:opacity-30"
